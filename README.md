...
与现场多出项及其原索引，免去人工逐项比对。**雨污遮挡**造成同一箱体被
识别出多份相互矛盾的十一位结果时，还可使用多读数共识入口：把 2..100
条原始读数一次提交，服务以候选字符对全部读数的逐位不一致总数为代价，
在**校验位约束下做全局动态规划**，给出最可信的合法箱号、最小代价、
并列最优解的精确总数与字典序前 100 个解，而**不是**先逐位取多数再
修补末位。
...
闸口摄像头受雨水、污渍遮挡时，同一只箱体的一次过闸可能被识别成多份
相互矛盾的十一位结果。人工取舍的常见做法是"逐位看多数，末位再按校验
码修一下"，但这会得到**局部最优**：某个位置的多数字符可能必须与其他
位置联动（校验位耦合全部前十位），修末位反而引入更多分歧。共识入口
因此在**合法箱号空间**内直接求**全局最优**：

端点：`POST /api/v1/container-numbers/consensus`
请求体：`{"container_numbers": [ ... ]}`，数组长度 **2 至 100**，每条
读数必须**恰为 11 个字符**；字符串原样使用，不做大小写或空白归一化。

### 候选域、代价与动态规划

1. **候选域**：逐位置收集该位置上**实际出现且符合该位置字符域**
   （1–3 位 `A-Z`、第 4 位 `U/J/Z`、5–11 位 `0-9`）的字符。非法
   字符（小写、越域字符、符号、代理字符等）**只计逐位不一致，永不
   进入候选域**；**重复读数按重复次数参与计票**。
2. **代价**：候选完整箱号的代价 = 它对**全部**读数的逐位不一致总数，
   即每一位"读数总数减去该候选字符在该位的出现次数"之和——非法观测在
   每个候选下都算一次不一致。
3. **动态规划**：以后缀 DP 在前 10 位加权和对 11 的余数（0..10）状态
   上推进，末位只接受由前缀余数唯一折叠出的校验数字，从而在**校验位
   约束下**求全局最小代价，并同时累计达到该代价的**精确解数**；最后
   利用后缀最优代价剪枝，按完整箱号字典序产出前 100 个最优解。整个
   过程不做"先逐位多数、后修补末位"。
...
curl -s -X POST http://localhost:8000/api/v1/container-numbers/consensus \
  -H 'Content-Type: application/json' \
  -d '{"container_numbers": ["CSQU3054383", "CSQU3054383", "DSQU3054383"]}'
```
...
存在多个并列最优合法箱号时，`solution_count` 给出**精确总数**（可能
大于 100），`solutions` 只承载按完整箱号**字典序**排列的前 100 个，
`truncated` 标记其后是否仍有同分最优解；每个解都附相同的最小代价：
...
```json
{
  ...
  "truncated": true,
  "solutions": [
    {"container_number": "MBBU1290291", "cost": 11}
  ]
}
```

（上例为两条 11 位全异读数；列表省略后 99 项，`solution_count=180`
而非 100，调用方应据 `truncated` 与 `solution_count` 转人工裁定。）

### 情形三：无解（`not_found`）

两类边界都返回 200 与 `status=not_found`，此时 `minimum_cost` 为
`null`、`solution_count` 为 0、`solutions` 为空、`truncated` 为
`false`：

- **任一位置无合法观测**：该位置出现的字符全部不符合字符域（例如
  首位只见小写字母，或末位只见字母），候选域无法覆盖该位置；
- **候选域中不存在满足校验位的组合**：各位置都有合法候选，但无论
  怎样组合，末位都不等于前十位加权和折叠出的校验数字。

### 局部多数违反校验位的例子

读数 `CSQU3054383 ×3`、`DSQU3054384 ×5`、`CSQU3054384 ×4`：逐位
多数（含末位）合成 `CSQU3054384`，而 `CSQU305438` 的期望校验位是
`3`，合成号非法；把末位"修补"成 `CSQU3054383` 的代价是 14。服务
返回唯一全局最优 `DSQU3054384`（自身合法），最小代价仅 10。

### 请求形状错误（HTTP 422, Pydantic `detail`）

读数少于 2 条或超过 100 条、任一读数不是恰为 11 位的字符串、字段
缺失 / 类型错误 / 多余字段，一律返回 FastAPI 标准
`{"detail": [...]}`；读数**内容**含非法字符不是请求形状错误（它们
按共识语义只计不一致）。
...
- 多读数共识：恰为 11 位时按普通非法观测处理——只在该位置计一次
  不一致，不进入候选域；若某位置的观测全部是这类字符则无解
  （`not_found`），不会在响应渲染阶段以 500 中断。
...
最小索引、结构/校验位两类原结论）四种场景；共识入口以独立**暴力
枚举参考**（建候选域后笛卡尔积 + 独立校验位复算 + 对全部读数汉明
距离求和）在 60 组随机含噪读数上断言动态规划的最小代价、精确解数
与字典序前 100 解完全一致，并钉死局部多数违反校验位时全局最优
（`DSQU3054384` 代价 10 对贪心修补代价 14）、超过 100 个同分最优
（精确 180 解、列表 100、`truncated`、字典序稳定）、重复读数计票、
两类无解边界（位置无合法观测 / 无校验位组合）、唯一共识、非法字符
与代理字符只计不一致且不进候选域以及全部请求形状 422 形态：
...
tests/
  ...
  test_consensus.py # 多读数共识测试（独立暴力参考 + 随机性质 + 全局最优/精确计数/稳定顺序/两类无解）
scripts/
