...
与现场多出项及其原索引，免去人工逐项比对。**闸口摄像头受雨污遮挡**、
同一箱体产生多份矛盾的十一位识别结果时，可提交多读数共识入口：以各
位置实际出现且符合字符域的字符为候选，通过动态规划在校验位约束下求
代价全局最优的合法箱号，并给出最优解总数与字典序前一百个解。
...
闸口摄像头受雨污遮挡时，同一箱体会被多次识别出**相互矛盾的十一位
读数**。逐位取多数再修补末位的做法并不可靠：多数派拼出的号码可能
本身违反校验位约束，而修补出的末位甚至可能从未在观测中出现。共识
入口把这一问题作为约束优化求解：

端点：`POST /api/v1/container-numbers/consensus`
请求体：`{"readings": [ ... ]}`，**2 至 100** 条原始读数，每条恰为
11 个字符，原样使用，不做大小写或空白归一化（字段缺失、读数非字符串、
长度非十一位、数量越界、多余字段均按请求校验返回 422 `detail`）。

求解口径（领域层一次完成，路由只做编排与字段映射）：

- **候选域**：每个位置的候选字符为该位置**实际出现且符合字符域**的
  字符（位置 1–3 取 `A-Z`，位置 4 取 `U/J/Z`，位置 5–11 取 `0-9`）。
  读数中的非法字符（小写、符号等）只在代价中计为不一致，**不进入
  候选域**；重复读数逐次计票。
- **代价**：候选箱号对全部读数的**逐位不一致总数**。
- **约束**：末位校验码必须等于前 10 位算出的期望校验位（余数 10
  折叠为 0），与批量校验同一来源。
- **算法**：按（位置, 加权和余数）的**动态规划**先算出全局最小代价
  与最优解精确总数，再只沿最优分支按字典序枚举前 100 个解——
  **不**先逐位取多数再修补末位。

响应字段：

- `status`：`determined` 唯一最优解（确定）/ `ambiguous` 多个最优解
  （歧义）/ `no_solution` 无解；
- `minimum_cost`：最优解的逐位不一致总数；**无解时为 `null`**；
- `solution_count`：达到最小代价的最优解**总数**（精确计数，不受
  返回上限影响）；
- `solutions`：按完整箱号字典序排列的**前 100 个**最优解；
- `truncated`：最优解总数超过 100、返回列表被截断时为 `true`。

```bash
curl -s -X POST http://localhost:8000/api/v1/container-numbers/consensus \
  -H 'Content-Type: application/json' \
  -d '{"readings": ["CSQU3054384", "CSQU3054384", "CSQU3054384",
       "CSQU3054784", "CSQU3054784"]}'
```

上例逐位多数派为 `CSQU3054384`——期望校验位 3 与实际末位 4 不符，
是非法箱号；而"修补末位"得到的 `CSQU3054383` 的末位 `3` 从未在观测
中出现，不在候选域内。全局最优解偏离多数派第 9 位（`3`→`7`）：
...
```json
{
  ...
  "minimum_cost": 3,
  "solution_count": 1,
  "solutions": ["CSQU3054784"],
  "truncated": false
}
```

歧义情形（两个合法箱号各观测一次，两个同分最优解按字典序返回）：
...
```json
{
  ...
  "minimum_cost": 2,
  "solution_count": 2,
  "solutions": ["CSQU3054383", "CSQU3054784"],
  "truncated": false
}
```

最优解总数超过 100 时只返回字典序前 100 个解并置 `truncated=true`，
`solution_count` 仍是精确总数。两种**无解**情形均返回
`status=no_solution`、`minimum_cost=null`、`solution_count=0`、
`solutions=[]`：任一位置无合法观测（如该位置全部读数都是非法字符），
或候选域中不存在满足校验位的组合。
...
- 多读数共识：恰为 11 位的读数合法参与计票，代理字符不属于任何
  字符域，只在代价中计为不一致、不产生候选。
...
最小索引、结构/校验位两类原结论）四种场景；共识入口以独立暴力枚举
参考断言全局最优（局部多数违反校验位时偏离多数派）、精确计数（超过
一百个同分最优时总数 294、只返回字典序前一百个解并置截断标记）、
稳定字典序、两类无解边界（位置无合法观测、无满足校验位的组合）、
唯一共识、重复读数计票与非法字符只计不一致，并以随机读数对照暴力
枚举逐字段复核：
...
tests/
  ...
  test_consensus.py # 多读数共识领域测试（全局最优、精确计数、截断、两类无解、随机对照暴力枚举）
